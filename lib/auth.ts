...
export function verifyWebhook(headers: Headers, rawBody: string): AuthResult {
    // TEMP: Allow all requests for MVP since user cannot add headers
    return { authorized: true, mode: 'none' };
    ...
    /*
      ...
      // Fallback for debugging if env var is missing
      const expectedToken = process.env.WEBHOOK_BEARER_TOKEN || 'vincit_secret_2025';
    
      // With a hardcoded fallback, expectedToken will always have a value,
      // so the previous check for !expectedToken is no longer needed.
